...
class App extends Component {
  state = {
    timer: 60,
    Score: 0,
    tabId1: tabsList[0].tabId,
    presentTab: imagesList[0].imageUrl,
    tab: 0,
  }
  ...
  getfilterprojects = () => {
    const {tabId1} = this.state
    const filtering = imagesList.filter(each => each.category === tabId1)
    return filtering
  }
  ...
  receivingTab = tabId => {
    ...
    this.setState({tabId1: tabId})
  }

  receivingNewProject = thumbnail => {
    const {presentTab, tab} = this.state
    const randomValue = Math.random() * 29
    const randomImage = Math.ceil(randomValue)

    if (presentTab === thumbnail) {
      this.setState(prevState => {
        const value = {
          Score: prevState.Score + 1,
          presentTab: imagesList[randomImage].imageUrl,
          tab: randomImage,
        }
        return value
      })
  ...
  render() {
    const {timer, Score, presentTab, tab} = this.state
    console.log(imagesList[tab])
    const filteredList = this.getfilterprojects()
    ...
    console.log(imagesList[tab].imageUrl)
    ...
    if (imagesList[tab].imageUrl !== presentTab) {
      return (
        <div className="background-image">
          <ul>
            <NavItem navItem={navItem} />
    ...
    return (
      <div className="background-image">
        ...
        <img src={presentTab} alt="match" className="size2" />
        ...
        <ul className="unorder">
          {tabsList.map(each => (
            <TabItem
              tabsList={each}
              key={each.tabId}
              receivingTab={this.receivingTab}
            />
          ))}
        </ul>
        <ul className="unorder">
          {filteredList.map(each => (
            <ProjectItems
              key={each.id}
              projectsList={each}
              receivingNewProject={this.receivingNewProject}
            />
          ))}
        </ul>
      </div>
